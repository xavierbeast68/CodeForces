...
signed main() {
    ...
    auto __solve_testcase = [&](int testcase) -> void {
        ...
        vector<int> nn(n), mm(m); cin >> nn >> mm;
        
        int nMin = *min_element(nn.begin(), nn.end());
        int nMax = *max_element(nn.begin(), nn.end());
        int mMin = *min_element(nn.begin(), nn.end());
        int mMax = *max_element(nn.begin(), nn.end());
