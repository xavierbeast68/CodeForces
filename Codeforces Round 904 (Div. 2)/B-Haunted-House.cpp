...
signed main() {
    ...
    auto __solve_testcase = [&](int testcase) -> void {
        ...
        for(int i = 1; i <= n; i++){
        	...
        	// else need to swap
        	size_t found = s.find('0', i+1);
        	// int found = lower_bound(indZero.begin(), indZero.end(), i+1) - indZero.begin();
        	// also requires to remove this index from indZero
