...
bool dfs(int ind, int n, vector<int> a, int currSum, int totalSum){
    if(ind == n){
        if(currSum == 0 || totalSum == 0){
            return false;
        }
        return (currSum%2 == totalSum%2);
    }

    if(currSum != 0){
        if(totalSum != 0 && currSum%2 == totalSum%2){
            return true;
        }
    }

    return dfs(ind+1, n, a, currSum + a[ind], totalSum - a[ind]) || dfs(ind+1, n, a, currSum,  totalSum);
}
...
signed main() {
    ...
    auto __solve_testcase = [&](int testcase) -> void {
        ...
        bool isPossible = dfs(0, n, a, 0, totalSum);
