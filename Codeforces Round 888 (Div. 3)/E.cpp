...
/*
    *Thought Process*
    ...
    18o3->  using toposort
    https://codeforces.com/contest/1851/submission/215562413

    tourist->
    https://codeforces.com/contest/1851/submission/215519516

    jiangly->
    https://codeforces.com/contest/1851/submission/215536264

    callmepandey->
    https://codeforces.com/contest/1851/submission/215567862

    shreyd01->
    https://codeforces.com/contest/1851/submission/215561660
*/
...
void solve(){
    ...
    readInt(n);readInt(k)
    vi buy(n); cin >> buy;
    vi unlimit(k); cin >> unlimit;
    
    vector<vector<int>> useOther;
    for(int i = 0; i < n; i++){
        readInt(sz);
        vi temp(sz); cin >> temp;
        useOther.pb(temp);
    ...
    FOR(i, 0, n){
        ...
        int makingCost = 0;
        // else
        for(int j = 0; j < useOther[i].size(); j++){
            makingCost += buy[useOther[i][j] - 1];
        }
        ans[i] = min(buy[i], makingCost);
    }

    for_each(all(ans), [](int i){
        cout << i << " ";
    });
    cendl;
}
