...
int ans = INT_MAX;
int computePower(int i, int j, vector<int> diff){
    if(i == j){
        return 0;
    }
...
void dfs(int ind, int k, int n, vector<int> arr, vector<int> diff, int currPower){
    if(k == 1){
        if(ind != n){
            currPower += computePower(ind, n-1, diff);
            ans = min(ans, currPower);
        }
        return;
    }

    for(int i = ind; i < n; i++){
        int curr = computePower(ind, i, diff);
        currPower += curr;
        dfs(i+1, k-1, n, arr, diff, currPower);
        currPower -= curr;
    }
}
void solve(){
