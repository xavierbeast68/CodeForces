...
signed main() {
    ...
    auto __solve_testcase = [&](int testcase) -> void {
        ...
        int di = vec[n-1].first, si = vec[n-1].second;
        int k = di + (si == 1 ? -1 : (ceil((double)si/2.0)-1));
        
        // cerr << k << endl;
        
        FOR(i, 0, n){
        	di = vec[i].first, si = vec[i].second;
        	int currLim = di + (si == 1 ? -1 : (ceil((double)si/2.0)-1));
        	// cout << di << " " << si << " " << currLim << endl;
