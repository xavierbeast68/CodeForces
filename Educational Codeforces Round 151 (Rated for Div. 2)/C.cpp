...
/*
    *Thought Process*
    ...
    https://www.youtube.com/watch?v=s_joFIyr3EI

    https://codeforces.com/contest/1845/submission/212123590
*/
...
void solve(){
    ...
    vector<int> v[10]; // stores indexes of numbers from 0 to 9
    for(int i = 0; i < n; i++){
        v[s[i]-'0'].push_back(i);
    }
    for(int i = 0; i <= 9; i++){
    ...
    int ind = -1; // initially we will start our search from the 0th index, so we need to stand before it
    int check = 0; // if we get our answer before exploring all combinations, then we break out of the loop
    for(int i = 0; i < m; i++){
        ...
        for(int j = low; j <= high; j++){
            // finding the next index of every jth number after ith indTH index
            int up_ind = upper_bound(v[j].begin(), v[j].end(), ind) - v[j].begin();
            ...
            if(v[j][up_ind] == n){
                ...
                newInd = max(newInd, v[j][up_ind]);
            }
        }
        ind = newInd;
    }

    if(check == 1)      println("YES");
    else                println("NO");
}
